...
export class BullQueuService implements QueueService {
  ...
  constructor() {
    ...
    this.purchaseConfirmationEmailQueue = new Queue(
  ...
  async sendPurchaseConfirmationEmail(params: {
    ...
    await this.purchaseConfirmationEmailQueue.add(
      "seller-purchase-confirmation-job",
      params,
    );
  }

  async sendLowerPriceNotification(params: {
    email: string;
    bookTitle: string;
    bookPrice: number;
  }): Promise<void> {
