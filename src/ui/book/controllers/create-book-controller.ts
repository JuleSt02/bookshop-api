...
const createBookValidationSchema = z.object({
  ...
  description: z.string().min(25, "Min length for description is 25 characters"),
  ...
  genre: z
    ...
    .pipe(
      z.enum(BookGenre, {
        error: "Genre has to be one of: Fantasy, Biography, Science Fiction, History or Novel",
      })
    ),
});
...
export const createBookController =  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const {title, description, author, price, genre} = createBookValidationSchema.parse(req.body)
